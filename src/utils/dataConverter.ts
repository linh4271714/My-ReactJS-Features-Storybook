...
export const stringToFile = (input: string, filename: string) => {
  if (!input) return;
...
export const blobToJson = (file: Blob) =>
  new Promise((resolve, reject) => {
    const reader = new FileReader();
...
export const convertSvgToBase64 = (src: string) => {
  return fetch(src)
...
export async function convertImageToFile(imageUrl: string) {
